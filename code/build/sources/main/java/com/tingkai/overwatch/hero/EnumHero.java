...
public enum EnumHero
{
	...
	public ItemModWeapon weapon;
	public ItemModToken token;
	...
	private EnumHero(String name, boolean hasAltWeapon, Ability ability1, Ability ability2, Ability ability3, int mainAmmo, int altAmmo, int[] armorReductionAmounts, ItemModWeapon weapon, Crosshair crosshair)
	{
	...
	public static class RenderManager
	{
		...
		public static void renderCrosshairs(RenderGameOverlayEvent.Pre event)
		{
			if (event.getType() == ElementType.CROSSHAIRS && Config.guiScale > 0)
			{
				...
				EnumHero hero = ItemModArmor.SetManager.playersWearingSets.containsKey(player.getPersistentID()) ? ItemModArmor.SetManager.playersWearingSets.get(player.getPersistentID()) : null;
				EnumHand hand = null;
				for (EnumHand hand2 : EnumHand.values())
				{
					if (player.getHeldItem(hand2) != null && player.getHeldItem(hand2).getItem() instanceof ItemModWeapon && (((ItemModWeapon) player.getHeldItem(hand2).getItem()).hero == hero || hand == null || ((ItemModWeapon) player.getHeldItem(hand).getItem()).hero != hero))
					{
						hand = hand2;
					}
				}
				ItemModWeapon weapon = hand == null ? null : (ItemModWeapon) player.getHeldItem(hand).getItem();
				...
				if (weapon != null)
				{
					if (!(weapon.hero == hero && Overwatch.keys.heroInformation(player)) && Minecraft.getMinecraft().gameSettings.thirdPersonView == 0)
					{
						...
						// tracer's dash
						if (weapon.hero == EnumHero.TRACER && ItemModArmor.SetManager.playersWearingSets.get(player.getPersistentID()) == EnumHero.TRACER)
						{
							GlStateManager.pushMatrix();
							GlStateManager.enableBlend();
						...
						else if (weapon.hero == EnumHero.REAPER && ItemReaperShotgun.clientTps.containsKey(player) && ItemReaperShotgun.clientTps.get(player).getFirst() == -1)
						{
							GlStateManager.pushMatrix();
							GlStateManager.enableBlend();
			...
			if (event.getType() == ElementType.HELMET && Config.guiScale > 0)
			{
				...
				else
				{
					...
					if (weapon != null)
					{
						...
						Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation(Overwatch.MODID, "textures/gui/ability_overlay.png"));
						int index = weapon.hero.playersUsingAlt.containsKey(player.getPersistentID()) && weapon.hero.playersUsingAlt.get(player.getPersistentID()) && weapon.hero.hasAltWeapon ? weapon.hero.altWeaponIndex : weapon.hero.overlayIndex;
						int vertical = 11;
						// weapon
						GuiUtils.drawTexturedModalRect(0, 0, 1, (index + 1) + index * vertical, 122, vertical, 0);

						if (hero != null && weapon.hero == hero && ItemModArmor.SetManager.playersWearingSets.containsKey(player.getPersistentID()))
						{
							// slot 1
