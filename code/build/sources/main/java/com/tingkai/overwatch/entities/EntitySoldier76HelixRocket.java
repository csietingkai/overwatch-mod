...
public class EntitySoldier76HelixRocket extends EntityModThrowable
{
	...
	protected void onImpact(RayTraceResult result)
	{
		...
		if (this.getThrower() != null && result.entityHit != this.getThrower() && !(result.entityHit instanceof EntitySoldier76HelixRocket) && (!(result.entityHit instanceof EntityLivingBase) || ((EntityLivingBase) result.entityHit).getHealth() > 0))
		{
			// direct hit damage (explosions do plenty of damage - direct can't
			// be much)
			if (result.entityHit instanceof EntityLivingBase && !this.world.isRemote)
			{
				((EntityLivingBase) result.entityHit).attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer) this.getThrower()), 1f * ItemModWeapon.damageScale);
			...
			if (this.world.isRemote)
			{
				Overwatch.proxy.spawnParticlesSmoke(world, posX, posY, posZ, 0x62E2FC, 0x203B7E, 25, 10);
			}
			this.setDead();
		}
	}
