		...
		public IMessage onMessage(final SPacketSpawnParticle packet, final MessageContext ctx)
		{
			...
			mainThread.addScheduledTask(new Runnable()
			{
				...
				public void run()
				{
					...
					if (packet.type == 0)
					{
						Overwatch.proxy.spawnParticlesSmoke(world, packet.x, packet.y, packet.z, packet.color, packet.colorFade, packet.scale, packet.maxAge);
					}
					else if (packet.type == 1)
					{
						Overwatch.proxy.spawnParticlesSpark(world, packet.x, packet.y, packet.z, packet.color, packet.colorFade, packet.scale, packet.maxAge);
					}
					else if (packet.type == 2)
					{
						for (int i = 0; i < 3; ++i)
						{
							Overwatch.proxy.spawnParticlesMeiBlaster(world, packet.x + world.rand.nextDouble() - 0.5d, packet.y + world.rand.nextDouble() - 0.5d, packet.z + world.rand.nextDouble() - 0.5d, 0, 0.01f, 0, world.rand.nextFloat(), 5, 20f, 25f);
						}
					}
				}
			});
