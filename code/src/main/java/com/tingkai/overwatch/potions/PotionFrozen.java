package com.tingkai.overwatch.potions;
...
public class PotionFrozen extends Potion
{
	...
	public void renderInventoryEffect(int x, int y, PotionEffect effect, net.minecraft.client.Minecraft mc)
	{
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation(Overwatch.MODID + ":textures/effects/frozen.png"));
	...
	public void removeAttributesModifiersFromEntity(EntityLivingBase entity, AbstractAttributeMap map, int amplifier)
	{
		...
		if (amplifier == 0)
		{
			Overwatch.network.sendToAll(new SPacketSpawnParticle(2, entity.posX, entity.posY + entity.height / 2, entity.posZ, 0, 0, 0, 0));
		}
	}
	...
	{
		if (clientFreezes.containsKey(event.getEntity()) || (event.getEntity().getActivePotionEffect(ModPotions.frozen) != null && event.getEntity().getActivePotionEffect(ModPotions.frozen).getDuration() > 0) && event.getEntity().getActivePotionEffect(ModPotions.frozen).getAmplifier() == 0)
		{
			...
			{
				event.getEntity().world.spawnParticle(EnumParticleTypes.SNOW_SHOVEL, (event.getEntity().posX + rand.nextDouble() - 0.5d) * event.getEntity().width, event.getEntity().posY + rand.nextDouble() - 0.5d + event.getEntity().height / 2, (event.getEntity().posZ + rand.nextDouble() - 0.5d) * event.getEntity().width, (rand.nextDouble() - 0.5d) * 0.5d, rand.nextDouble() - 0.5d, (rand.nextDouble() - 0.5d) * 0.5d, new int[0]);
			}
			if (rand.nextInt(70 - freeze * 2) == 0)
			{
				Overwatch.proxy.spawnParticlesMeiBlaster(event.getEntity().world, event.getEntity().posX + rand.nextDouble() - 0.5d, event.getEntity().posY + rand.nextDouble() - 0.5d + event.getEntity().height / 2, event.getEntity().posZ + rand.nextDouble() - 0.5d, 0, (rand.nextDouble()) * 0.2f, 0, rand.nextFloat(), 8, 2.5f, 2f);
			}
		}
	}
	...
	public void clientSide(PlayerTickEvent event)
	{
		...
		if (event.phase == TickEvent.Phase.START && event.side == Side.CLIENT && event.player.getActivePotionEffect(ModPotions.frozen) != null && event.player.getActivePotionEffect(ModPotions.frozen).getDuration() > 0)
		{
		...
		{
			if (event.player.getActivePotionEffect(ModPotions.frozen) != null && event.player.getActivePotionEffect(ModPotions.frozen).getDuration() == 0)
			{
			...
			if (event.player.ticksExisted % 2 == 0)
			{
				...
				{
					if (clientFreezes.get(entity) > 1 && !entity.isDead && (entity.getActivePotionEffect(ModPotions.frozen) == null || entity.getActivePotionEffect(ModPotions.frozen).getDuration() == 0))
					{
	...
	public void serverSide(WorldTickEvent event)
	{
		if (event.phase == TickEvent.Phase.END && event.world.getTotalWorldTime() % 6 == 0)
		{
			...
			{
				if (serverFreezes.get(entity) > 1 && !entity.isDead && (entity.getActivePotionEffect(ModPotions.frozen) == null || entity.getActivePotionEffect(ModPotions.frozen).getDuration() == 0))
				{
					...
					if (serverFreezes.get(entity) >= 30)
					{
						...
						Overwatch.network.sendToAll(new SPacketPotionEffect(entity, effect));
						Overwatch.network.sendToAll(new SPacketSpawnParticle(2, entity.posX, entity.posY + entity.height / 2, entity.posZ, 0, 0, 0, 0));
	...
	public void mouseEvent(MouseEvent event)
	{
		...
		if (event.isButtonstate() && player != null && (player.getActivePotionEffect(ModPotions.frozen) != null && player.getActivePotionEffect(ModPotions.frozen).getDuration() > 0))
		{
	...
	{
		EntityPlayer player = Minecraft.getMinecraft().player;
		if (player != null && rotations.containsKey(player.getPersistentID()) && (player.getActivePotionEffect(ModPotions.frozen) != null && player.getActivePotionEffect(ModPotions.frozen).getDuration() > 0))
		{
	...
	public void setEntityRotations(LivingUpdateEvent event)
	{
		...
		if (triple != null)
		{
			if (event.getEntityLiving().getActivePotionEffect(ModPotions.frozen) == null || event.getEntityLiving().getActivePotionEffect(ModPotions.frozen).getDuration() == 0)
			{
	...
	{
		if (event.getSource().getEntity() instanceof EntityLivingBase && ((EntityLivingBase) event.getSource().getEntity()).getActivePotionEffect(ModPotions.frozen) != null && ((EntityLivingBase) event.getSource().getEntity()).getActivePotionEffect(ModPotions.frozen).getDuration() > 0)
		{
	...
	{
		if (event.getEntity() instanceof EntityLivingBase && ((EntityLivingBase) event.getEntity()).getActivePotionEffect(ModPotions.frozen) != null && ((EntityLivingBase) event.getEntity()).getActivePotionEffect(ModPotions.frozen).getDuration() > 0)
		{
	...
	{
		if (event.getEntity() instanceof EntityLivingBase && ((((EntityLivingBase) event.getEntity()).getActivePotionEffect(ModPotions.frozen) != null && ((EntityLivingBase) event.getEntity()).getActivePotionEffect(ModPotions.frozen).getDuration() > 0) || event.getEntity() instanceof EntityEnderman && (clientFreezes.containsKey(event.getEntity()) || serverFreezes.containsKey(event.getEntity()))))
		{
			event.setCanceled(true);
		}
	}
	...
	{
		if (event.getTarget() != null && event.getEntity() instanceof EntityLivingBase && ((EntityLivingBase) event.getEntity()).getActivePotionEffect(ModPotions.frozen) != null && ((EntityLivingBase) event.getEntity()).getActivePotionEffect(ModPotions.frozen).getDuration() > 0)
		{
			if (event.getEntity() instanceof EntityLiving)
			{
