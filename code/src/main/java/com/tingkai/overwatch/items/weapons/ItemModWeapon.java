...
import com.tingkai.overwatch.items.armors.ItemModArmor.SetManager;
...
public abstract class ItemModWeapon extends Item
{
	...
	public ItemModWeapon(int reloadTime)
	{
	...
	{
		if (player != null && hero.hasAltWeapon && hero.playersUsingAlt.containsKey(player.getPersistentID()) && hero.playersUsingAlt.get(player.getPersistentID()))
		{
			return hero.altAmmo;
		}
		else
	...
	public void setCurrentAmmo(EntityPlayer player, int amount, EnumHand... hands)
	{
		if (player != null)
		{
			...
			{
				EnumHand hand = player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() == this ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND;
				Overwatch.network.sendTo(new SPacketSyncAmmo(player.getPersistentID(), hand, amount, hands), (EntityPlayerMP) player);
			}
	...
	{
		if (player == null || player.getCooldownTracker().hasCooldown(this) || (this.getMaxAmmo(player) > 0 && this.getCurrentAmmo(player) == 0) || (player.getActivePotionEffect(ModPotions.frozen) != null && player.getActivePotionEffect(ModPotions.frozen).getDuration() > 0))
		{
			return false;
		}
		...
		{
			if (shouldWarn && (!this.warningCooldown.containsKey(player.getPersistentID()) || this.warningCooldown.get(player.getPersistentID()) == 0))
			{
				player.sendMessage(new TextComponentString(TextFormatting.RED + new ItemStack(this).getDisplayName() + " must be held in the main-hand and off-hand to work."));
			}
		}
		else if (main == null || main.getItem() != this)
		{
			if (shouldWarn && (!this.warningCooldown.containsKey(player.getPersistentID()) || this.warningCooldown.get(player.getPersistentID()) == 0))
			{
				player.sendMessage(new TextComponentString(TextFormatting.RED + new ItemStack(this).getDisplayName() + " must be held in the main-hand to work."));
			}
		}
		else
		{
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int slot, boolean isSelected)
	{
		...
		// reloading
		if (!world.isRemote && entity instanceof EntityPlayer && (((EntityPlayer) entity).getHeldItemMainhand() == stack || ((EntityPlayer) entity).getHeldItemOffhand() == stack))
			// automatic reload
			if (this.getCurrentAmmo((EntityPlayer) entity) == 0 && this.getMaxAmmo((EntityPlayer) entity) > 0 && !((EntityPlayer) entity).getCooldownTracker().hasCooldown(this))
			this.setCurrentAmmo((EntityPlayer) entity, this.getMaxAmmo((EntityPlayer) entity), EnumHand.values());
			// manual reload
			else if (this.getCurrentAmmo((EntityPlayer) entity) > 0 && Overwatch.keys.reload((EntityPlayer) entity))
		...
		if (entity instanceof EntityPlayer && Overwatch.keys.lmb((EntityPlayer) entity))
		{
			for (EnumHand hand : EnumHand.values())
			{
				if (((EntityPlayer) entity).getHeldItem(hand) == stack && (!hasOffhand || !(hand == EnumHand.MAIN_HAND && entity.ticksExisted % 2 == 0)))
				{
		...
		if (entity instanceof EntityPlayer)
		{
			...
			{
				if (ability.keybind.getCooldown((EntityPlayer) entity) > 0 && ability.toggled.containsKey(entity.getPersistentID()) && ability.toggled.get(entity.getPersistentID()))
				{
		...
		else if (!world.isRemote && Config.durabilityOptionWeapons == 1 && stack.getItemDamage() != 0 && SetManager.playersWearingSets.get(entity.getPersistentID()) == hero)
		{
			stack.setItemDamage(0);
		}
	}
