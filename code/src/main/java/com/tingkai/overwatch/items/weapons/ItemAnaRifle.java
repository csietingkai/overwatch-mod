...
public class ItemAnaRifle extends ItemModWeapon
{
	private static final ResourceLocation SCOPE = new ResourceLocation(Overwatch.MODID + ":textures/gui/ana_scope.png");
	private static final ResourceLocation SCOPE_BACKGROUND = new ResourceLocation(Overwatch.MODID + ":textures/gui/ana_scope_background.png");
	...
	public void onItemLeftClick(ItemStack stack, World world, EntityPlayer player, EnumHand hand)
	{
		if (this.canUse(player, true))
		{
			if (!world.isRemote)
			{
				EntityAnaBullet bullet = new EntityAnaBullet(world, player, hero.playersUsingAlt.containsKey(player.getPersistentID()) && hero.playersUsingAlt.get(player.getPersistentID()));
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int itemSlot, boolean isSelected)
	{
		...
		// health particles
		if (isSelected && entity instanceof EntityPlayer && this.canUse((EntityPlayer) entity, false) && world.isRemote && entity.ticksExisted % 5 == 0)
		{
			...
			{
				if (entity2 instanceof EntityLivingBase && ((EntityLivingBase) entity2).getHealth() < ((EntityLivingBase) entity2).getMaxHealth())
				{
					Overwatch.proxy.spawnParticlesAnaHealth((EntityLivingBase) entity2);
				}
			}
		}

		// scope while right click
		if (entity instanceof EntityPlayer && ((EntityPlayer) entity).getActiveItemStack() != stack && Overwatch.keys.rmb((EntityPlayer) entity) && isSelected && this.getCurrentAmmo((EntityPlayer) entity) > 0)
		{
	...
	public void renderScope(RenderGameOverlayEvent.Pre event)
	{
		...
		if (event.getType() == ElementType.ALL && player != null && player.getHeldItemMainhand() != null && player.getHeldItemMainhand().getItem() == this && Overwatch.keys.rmb(player) && Minecraft.getMinecraft().gameSettings.thirdPersonView == 0 && this.getCurrentAmmo((EntityPlayer) player) > 0)
		{
