...
public class Ability
{
	...
	{
		if (player.world.isRemote && this.keybind.getCooldown(player) > 0 && keybind.isKeyDown(player) && !this.keybind.abilityNotReadyCooldowns.containsKey(player.getPersistentID()))
		{
	...
	{
		if (player.world.isRemote && keybind.getCooldown(player) > 0 && keybind.isKeyDown(player) && !keybind.abilityNotReadyCooldowns.containsKey(player.getPersistentID()))
		{
			keybind.abilityNotReadyCooldowns.put(player.getPersistentID(), 20);
		}

		boolean ret = (maxUses == 0 || getUses(player) > 0) && ((player.getActivePotionEffect(ModPotions.frozen) == null || player.getActivePotionEffect(ModPotions.frozen).getDuration() == 0) && ItemModArmor.SetManager.playersWearingSets.containsKey(player.getPersistentID()) && ItemModArmor.SetManager.playersWearingSets.get(player.getPersistentID()) == hero) && keybind.getCooldown(player) == 0 && (keybind.isKeyDown(player) || (toggled.containsKey(player.getPersistentID()) && toggled.get(player.getPersistentID())));
	...
	public void subtractUse(EntityPlayer player)
	{
		if (player != null && !player.world.isRemote && getUses(player) > 0 && player instanceof EntityPlayerMP)
		{
			...
			Overwatch.network.sendTo(new SPacketSyncAbilityUses(player.getPersistentID(), hero, getNumber(), multiAbilityUses.get(player.getPersistentID()), false), (EntityPlayerMP) player);
		}
	}
			...
			for (UUID uuid : multiAbilityCooldowns.keySet())
			{
				if (uuid.equals(event.player.getPersistentID()))
				{
					...
					else
					{
						...
						{
							Overwatch.network.sendTo(new SPacketSyncAbilityUses(event.player.getPersistentID(), hero, getNumber(), multiAbilityUses.get(event.player.getPersistentID()), true), (EntityPlayerMP) event.player);
						}
					}
					break;
				}
