...
public class Keys
{
	...
	public void mouseEvents(MouseEvent event)
	{
		...
		if ((event.isButtonstate() && ((main != null && main.getItem() instanceof ItemModWeapon) || (off != null && off.getItem() instanceof ItemModWeapon))) || !event.isButtonstate())
		{
			if (event.getButton() == 0)
			{
				...
				if (event.isButtonstate())
				{
					...
					{
						if (((ItemModWeapon) main.getItem()).canUse(player, false))
						{
							((ItemModWeapon) main.getItem()).onItemLeftClick(main, player.world, player, EnumHand.MAIN_HAND);
						}
						event.setCanceled(true);
					}
				}
			...
			if (event.getButton() == 1)
			{
				...
				Overwatch.network.sendToServer(new CPacketSyncKeys("RMB", event.isButtonstate(), uuid));
			}
		}

		if (main != null && main.getItem() instanceof ItemModWeapon && player.isSneaking() && event.getDwheel() != 0 && ((ItemModWeapon) main.getItem()).hero.hasAltWeapon)
		{
			EnumHero hero = ((ItemModWeapon) main.getItem()).hero;
			hero.playersUsingAlt.put(uuid, hero.playersUsingAlt.containsKey(uuid) ? !hero.playersUsingAlt.get(uuid) : true);
			Overwatch.network.sendToServer(new CPacketSyncKeys("Alt Weapon", hero.playersUsingAlt.get(uuid), uuid));
			event.setCanceled(true);
		}
	}
	...
	public void updateKeys(ClientTickEvent event)
	{
		if (event.phase == Phase.END && Minecraft.getMinecraft().player != null)
		{
			...
			if (Minecraft.getMinecraft().currentScreen != null && this.lmb(Minecraft.getMinecraft().player))
			{
				...
				Overwatch.network.sendToServer(new CPacketSyncKeys("LMB", false, player));
			}
			...
			if (Minecraft.getMinecraft().currentScreen != null && this.rmb(Minecraft.getMinecraft().player))
			{
				...
				Overwatch.network.sendToServer(new CPacketSyncKeys("RMB", false, player));
			}
			...
			if (!heroInformation.containsKey(player) || HERO_INFORMATION.isKeyDown() != heroInformation.get(player))
			{
				...
				Overwatch.network.sendToServer(new CPacketSyncKeys("Hero Information", HERO_INFORMATION.isKeyDown(), player));
			}
			if (!reload.containsKey(player) || RELOAD.isKeyDown() != reload.get(player))
			{
				reload.put(player, RELOAD.isKeyDown());
				Overwatch.network.sendToServer(new CPacketSyncKeys("Reload", RELOAD.isKeyDown(), player));
			}
			if (!ability1.containsKey(player) || ABILITY_1.isKeyDown() != ability1.get(player))
			{
				ability1.put(player, ABILITY_1.isKeyDown());
				Overwatch.network.sendToServer(new CPacketSyncKeys("Ability 1", ABILITY_1.isKeyDown(), player));
				// toggle ability
				if (ABILITY_1.isKeyDown() && ItemModArmor.SetManager.playersWearingSets.containsKey(player))
				{
					EnumHero hero = ItemModArmor.SetManager.playersWearingSets.get(player);
					for (Ability ability : new Ability[] { hero.ability1, hero.ability2, hero.ability3 })
						{if (ability.isToggleable && ability.keybind == KeyBind.ABILITY_1 && ability.keybind.getCooldown(Minecraft.getMinecraft().player) == 0)
						{
							...
							Overwatch.network.sendToServer(new CPacketSyncKeys("Toggle Ability 1", toggle, player));
						}}
				}
			}
			if (!ability2.containsKey(player) || ABILITY_2.isKeyDown() != ability2.get(player))
			{
				...
				Overwatch.network.sendToServer(new CPacketSyncKeys("Ability 2", ABILITY_2.isKeyDown(), player));
				// toggle ability
				if (ABILITY_2.isKeyDown() && ItemModArmor.SetManager.playersWearingSets.containsKey(player))
				{
					EnumHero hero = ItemModArmor.SetManager.playersWearingSets.get(player);
					for (Ability ability : new Ability[] { hero.ability1, hero.ability2, hero.ability3 })
						{if (ability.isToggleable && ability.keybind == KeyBind.ABILITY_2 && ability.keybind.getCooldown(Minecraft.getMinecraft().player) == 0)
						{
							boolean toggle = ability.toggled.containsKey(player) ? !ability.toggled.get(player) : true;
							hero.weapon.toggle(Minecraft.getMinecraft().player, ability, toggle);
							Overwatch.network.sendToServer(new CPacketSyncKeys("Toggle Ability 2", toggle, player));
						}}
				}
			}
			if (!ultimate.containsKey(player) || ULTIMATE.isKeyDown() != ultimate.get(player))
			{
				...
				Overwatch.network.sendToServer(new CPacketSyncKeys("Ultimate", ULTIMATE.isKeyDown(), player));
			}
		}
	}
	...
	public enum KeyBind
	{
		...
		public boolean isKeyDown(EntityPlayer player)
		{
			switch (this)
			{
				...
				case RMB:
					return Overwatch.keys.rmb(player);
				default:
					return false;
			}
		}
