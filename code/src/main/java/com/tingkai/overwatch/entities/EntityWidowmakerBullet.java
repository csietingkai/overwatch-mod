...
public class EntityWidowmakerBullet extends EntityModThrowable
{
	...
	public void onUpdate()
	{
		...
		else
		{
			if (this.world.isRemote)
			{
				...
				for (int i = 0; i < numParticles; ++i)
				{
					Overwatch.proxy.spawnParticlesTrail(this.world, this.posX + (this.prevPosX - this.posX) * i / numParticles, this.posY + this.height / 2 + (this.prevPosY - this.posY) * i / numParticles, this.posZ + (this.prevPosZ - this.posZ) * i / numParticles, 0, 0, 0, 0xFF0000, 0xFF0000, 0.5f, 1, 0.01f);
				}
			}
		}
	}
