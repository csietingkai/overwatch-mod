...
public class EntityHanzoSonicArrow extends EntityHanzoArrow
{
	...
	public static boolean doEffect(World world, Entity ignoreEntity, Entity trackEntity, double x, double y, double z, int ticks)
	{
		...
		if (world.isRemote && (ticks == 1 || ticks == 7 || ticks == 10 || ticks == 13))
		{
			if (trackEntity != null)
			{
				Overwatch.proxy.spawnParticlesHanzoSonic(world, trackEntity, true);
			}
			else
			{
				Overwatch.proxy.spawnParticlesHanzoSonic(world, x, y, z, true, false);
			}
		}
		else if (world.isRemote && ticks > 30 && ticks < 170 && (ticks % 30 == 0 || ticks % 32 == 0))
		{
			if (trackEntity != null)
			{
				Overwatch.proxy.spawnParticlesHanzoSonic(world, trackEntity, false);
			}
			else
			{
				Overwatch.proxy.spawnParticlesHanzoSonic(world, x, y, z, false, false);
			}
		}
		else if (ticks > 200)
		{
	...
	public void onUpdate()
	{
		...
		else if (!this.inGround && this.world.isRemote)
		{
			...
			{
				Overwatch.proxy.spawnParticlesHanzoSonic(world, posX, posY, posZ, false, true);
			}
			int numParticles = (int) ((Math.abs(motionX) + Math.abs(motionY) + Math.abs(motionZ)) * 5d);
			for (int i = 0; i < numParticles; ++i)
			{
				Overwatch.proxy.spawnParticlesTrail(this.world, this.posX + (this.lastTickPosX - this.posX) * i / numParticles + world.rand.nextDouble() * 0.05d, this.posY + (this.lastTickPosY - this.posY) * i / numParticles + world.rand.nextDouble() * 0.05d, this.posZ + (this.lastTickPosZ - this.posZ) * i / numParticles + world.rand.nextDouble() * 0.05d, 0, 0, 0, 0x5EDCE5, 0x007acc, 1, 20, 1);
			}
		}
	}
	...
	private static class EntityHitHandler
	{
		...
		public void clientSide(PlayerTickEvent event)
		{
			if (event.phase == TickEvent.Phase.END && event.side == Side.CLIENT)
			{
				...
				{
					if (entity == null || entity.isDead || doEffect(entity.world, null, entity, entity.posX, entity.posY, entity.posZ, clientHitEntities.get(entity)))
					{
						toRemove.add(entity);
					}
					else
		...
		public void serverSide(WorldTickEvent event)
		{
			if (event.phase == TickEvent.Phase.END && event.world.getTotalWorldTime() % 3 == 0)
			{
				...
				{
					if (entity == null || entity.isDead || doEffect(entity.world, null, entity, entity.posX, entity.posY, entity.posZ, serverHitEntities.get(entity)))
					{
						toRemove.add(entity);
					}
					else
